...
def connect_db(retries=5, delay=5):
    for i in range(retries):
        try:
            print(f"🔌 Attempt {i+1} connecting to DB:", os.getenv("DB_HOST"))
            return mysql.connector.connect(
                ...
                ssl_disabled=True  # use this unless Render says SSL is required
            )
        except mysql.connector.Error as err:
            print(f"⚠️ DB connection failed: {err}, retrying in {delay}s...")
            time.sleep(delay)
...
# print("Connecting to DB:", os.getenv("DB_HOST"))
